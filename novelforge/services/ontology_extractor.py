...
class OntologyExtractor:
        ...
        for attempt, temperature in enumerate(temperatures):
            ...
            try:
                ...
                if not choices:
                    ...
                    logger.warning(
                        "底层世界观汇总第 %d 次尝试无 choices", attempt + 1
                    )
                    continue
    ...
    async def extract_ontology_streaming(
        ...
        2. 逐批：构建含 ``{{accumulated_ontology}}`` 的提示词
           （首批注入"（首批提取，无前序参考）"），调用 LLM，解析 JSON → dict，
           通过 ``_merge_ontology_fields`` 合并到 accumulated_ontology
        3. 若 batch_count > 1：调用 ``_run_ontology_merge`` 语义整合
        4. 构建 WorldOntology（含 extracted_at 与 source_chapter_range）
        5. 保存到 ``Project.world_ontology`` + 世界书条目
        6. 返回 (WorldOntology, 状态消息)
