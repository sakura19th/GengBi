...
实现文风档案（Style Profile）的提取：
- 取小说全文按 token 限制拆分批次
- 每批次调用 LLM 提取 9 大维度参数化描述
- 多批次时通过【信息汇总】环节语义整合为最终 StyleProfile
- 提取结果固化为 Project.style_profile

三大核心机制（镜像 ContextExtractor / OntologyExtractor）：
1. 按 token 自动拆分：超 token_limit 时按章节边界拆分批次
2. 增量更新：每批次提取时携带前一批的 accumulated StyleProfile 作为参考
3. 合并功能：batch_count > 1 时调用 _run_style_merge 语义整合
"""
...
from novelforge.utils.paths import (
    get_extract_style_merge_prompt_path,
    get_extract_style_prompt_path,
)

logger = logging.getLogger(__name__)

# 提取请求的 max_tokens（足够返回 9 维度 JSON）
STYLE_EXTRACT_MAX_TOKENS = 8000

# 提取温度（低温保证稳定输出）
STYLE_EXTRACT_TEMPERATURE = 0.2

# 提取温度（重试，温度归零确保稳定）
STYLE_EXTRACT_TEMPERATURE_RETRY = 0.0

# 合并温度（首次尝试）
STYLE_MERGE_TEMPERATURE = 0.2

# 合并温度（重试，温度归零确保稳定）
STYLE_MERGE_TEMPERATURE_RETRY = 0.0

# StyleProfile 9 大维度字段名
...
def _filter_style_dimensions(data: dict[str, Any]) -> dict[str, Any]:
    """过滤字典，仅保留 9 大维度字段，非 dict 值替换为空 dict。
...
def _parse_style_response(content: str) -> dict[str, Any]:
    """解析 LLM 提取响应为 StyleProfile 字典。
    ...
    try:
        ...
        if isinstance(data, dict):
            return _filter_style_dimensions(data)
...
class StyleExtractor:
    ...
    全文拆分分析，提取 StyleProfile 9 大维度参数化描述。
    提取流程：全文按 token 拆分批次 → 逐批增量提取 → 合并 → 存储为 Project.style_profile。
    ...
    def _load_style_prompt_template(self) -> str:
        """加载文风档案提取提示词模板（带缓存）。
        ...
        if self._style_prompt_template is None:
            path = get_extract_style_prompt_path()
            try:
                self._style_prompt_template = path.read_text(encoding="utf-8")
            except OSError as e:
                logger.error("加载文风档案提取提示词模板失败: %s", e)
                # 回退最小可用模板
                self._style_prompt_template = (
                    "请分析以下小说全文，提取文风档案（Style Profile）。\n\n"
                    "# 小说档案\n标题：{{title}}\n作者：{{author}}\n"
                    "主角：{{protagonist}}\n简介：{{synopsis}}\n"
                    "世界观设定：{{world_setting}}\n写作风格：{{writing_style}}\n\n"
                    "# 前序批次累积的文风认知（增量上下文）\n"
                    "{{accumulated_style}}\n\n"
                    "# 本批次章节内容\n{{chapters_text}}\n\n"
                    "请输出严格 JSON 对象，含 9 大维度字段："
                    "language_texture/narrative_rhythm/scene_construction/"
                    "character_portrayal/emotion_engagement/innovation_signature/"
                    "protagonist_supporting_ratio/perspective_usage/time_transition。"
                    "只输出 JSON，不要 markdown 代码块。"
                )
        return self._style_prompt_template

    def _load_style_merge_prompt_template(self) -> str:
        """加载文风档案合并提示词模板（带缓存）。
        ...
        if self._style_merge_prompt_template is None:
            path = get_extract_style_merge_prompt_path()
            try:
                self._style_merge_prompt_template = path.read_text(
                    encoding="utf-8"
                )
            except OSError as e:
                logger.error("加载文风档案合并提示词模板失败: %s", e)
                # 回退最小可用模板
                self._style_merge_prompt_template = (
                    "请将以下多批次提取的 StyleProfile 合并为统一的 JSON 对象。\n\n"
                    "# 待合并的多批次提取结果\n{{entries_blocks}}\n\n"
                    "合并任务：消除跨块重复、冲突消解（以较新批次为准）、"
                    "补全与连贯、参数纯度（聚焦文笔风格量化参数）。"
                    "输出严格 JSON 对象，含 9 大维度字段。"
                    "只输出 JSON，不要 markdown 代码块。"
                )
    ...
    def _build_style_prompt(
        ...
        project: Project,
        chapters_text: str,
        accumulated_style: dict | None = None,
    ) -> str:
        """构建文风档案提取提示词。

        用宏替换填充 ``{{title}}``/``{{author}}``/``{{protagonist}}``
        /``{{synopsis}}``/``{{world_setting}}``/``{{writing_style}}``
        /``{{accumulated_style}}``/``{{chapters_text}}``。

        首批提取（accumulated_style=None 或空 dict）注入"（首批提取，无前序参考）"，
        后续批次注入累积 StyleProfile JSON。

        Args:
            project: 项目对象
            chapters_text: 本批次章节文本
            accumulated_style: 前序批次累积的 StyleProfile 字典（None 表示首批）

        Returns:
            填充后的提示词
        """
        template = self._load_style_prompt_template()
        ...
        # 累积 style 文本（首批注入提示文字）
        if not accumulated_style:
            accumulated_text = "（首批提取，无前序参考）"
        else:
            try:
                accumulated_text = json.dumps(
                    accumulated_style, ensure_ascii=False, indent=2
                )
            except (TypeError, ValueError) as e:
                logger.warning("序列化累积 style 失败，使用提示文字: %s", e)
        ...
        replacements = {
            ...
            "{{accumulated_style}}": accumulated_text,
    ...
    def _get_llm_client(self, flow_key: str = "") -> tuple[LLMClient, str] | None:
        ...
        Args:
            flow_key: 流程键（如 "style_extraction"），空串用默认端点
        ...
        if not ep:
            logger.error("未配置 API 端点，无法提取文风档案")
            return None

        api_key = self.config_manager.decrypt_api_key(ep.get("id", ""))
        if not api_key:
            logger.error("API Key 无效，无法提取文风档案")
            return None

        base_url = ep.get("base_url", "")
        if not base_url:
            logger.error("API base_url 为空，无法提取文风档案")
            return None
    ...
    def _merge_style_fields(
        self, accumulated: dict, new_batch: dict
    ) -> dict:
        """程序化字段级合并（增量更新核心）。

        对 StyleProfile 9 大维度逐字段合并：
        - 空字段：取另一侧
        - 非空：取更完整描述（按序列化长度启发式）
        - 冲突：新批次优先（后文更能反映风格全貌）

        Args:
            accumulated: 已累积的 StyleProfile 字典
            new_batch: 本批次提取的 StyleProfile 字典

        Returns:
            合并后的 StyleProfile 字典（含 9 大维度字段）
        """
        result: dict[str, Any] = {}
        for dim in STYLE_DIMENSIONS:
    ...
    def _build_entries_blocks(
        ...
        Args:
            batch_results: 各批次独立提取的 StyleProfile 字典列表
        ...
        parts: list[str] = []
        for idx, (style, (start, end)) in enumerate(
            zip(batch_results, batch_ranges)
        ):
            block_json = json.dumps(style, ensure_ascii=False, indent=2)
            parts.append(
    ...
    async def _run_style_merge(
        self,
        batch_results: list[dict],
        accumulated_style: dict,
        ...
        """合并多批次 StyleProfile 结果（【信息汇总】环节）。

        构造合并提示词（``extract_style_merge_prompt.txt``），调用 LLM，
        解析响应为 StyleProfile 字典。2 次重试（温度 0.2/0.0），
        失败返回 accumulated_style（降级）。

        Args:
            batch_results: 各批次独立提取的 StyleProfile 字典列表
            accumulated_style: 程序化合并的累积 style（降级时返回）
            llm_client: LLM 客户端
            model: 模型名
        ...
        Returns:
            合并后的 StyleProfile 字典，失败返回 accumulated_style（降级）
        """
        # 过滤空批次
        non_empty_indices = [i for i, r in enumerate(batch_results) if r]
        if not non_empty_indices:
            logger.warning("文风档案汇总：所有批次均为空，返回累积结果")
        ...
        template = self._load_style_merge_prompt_template()
        ...
        # 2 次重试：温度 0.2 / 0.0
        temperatures = [STYLE_MERGE_TEMPERATURE, STYLE_MERGE_TEMPERATURE_RETRY]
        last_error: str = ""
        ...
        # 全部重试失败 → 降级返回累积结果
        logger.warning(
            "文风档案汇总 2 次重试均失败，降级返回累积结果: %s", last_error
        )
        return accumulated_style

    # ===== 主流程：流式提取 =====

    async def extract_style_streaming(
        self,
        project: Project,
        chapters: list[Chapter],
        token_limit: int = 0,
        ...
        Returns:
            ...
            - 成功：(StyleProfile, 成功消息)
            - 失败：(None, 错误消息)
            - 取消：(None, "用户取消提取")
        """
        # 0 章时跳过提取
        if not chapters:
            logger.info("无章节可提取文风档案")
            return None, "无章节可提取"

        # 获取 LLM 客户端
        client_info = self._get_llm_client("style_extraction")
        ...
        try:
            ...
            accumulated_style: dict[str, Any] = {}
            ...
            for batch_idx, batch_chapters in enumerate(batches):
                # 取消信号检查
                if stop_event is not None and stop_event.is_set():
                    logger.info("文风档案提取被取消（批次 %d/%d 前）",
                ...
                try:
                    prompt = self._build_style_prompt(
                        ...
                        accumulated_style=accumulated_style or None,
                    )
                except Exception as e:
                    logger.error("构建文风档案提示词失败: %s", e, exc_info=True)
                ...
                extract_temperatures = [
                    STYLE_EXTRACT_TEMPERATURE,
                    STYLE_EXTRACT_TEMPERATURE_RETRY,
                ]
                batch_style: dict[str, Any] | None = None
                last_error = ""
                ...
                # 记录本批次独立结果（供汇总环节使用）
                # 循环仅 break（成功）或 return（失败）退出，此处 batch_style 必非 None
                assert batch_style is not None
                batch_results.append(batch_style)

                # 增量合并到 accumulated_style（核心：增量更新）
                accumulated_style = self._merge_style_fields(
                    accumulated_style, batch_style
                )
                ...
                logger.info(
                    ...
                    sum(1 for v in accumulated_style.values() if v),
                )
            ...
            # 【信息汇总】环节：batch_count > 1 时调用 _run_style_merge 语义整合
            merged = False
            if batch_count > 1:
                merged_style = await self._run_style_merge(
                    batch_results=batch_results,
                    accumulated_style=accumulated_style,
                    llm_client=client,
                    model=model,
                    on_chunk=on_chunk,
                    stop_event=stop_event,
                ...
                if merged_style is not None:
                    accumulated_style = merged_style
                    merged = True
                    logger.info("文风档案【信息汇总】环节完成")
                else:
                    # 汇总失败：降级使用累积结果（_run_style_merge 内部已降级处理）
                    logger.warning("文风档案【信息汇总】环节失败，降级使用累积结果")
            ...
            try:
                ...
                logger.error("保存文风档案到项目失败: %s", e, exc_info=True)
                # 保存失败仍返回 style_profile（已在内存中构造完成）
                return style_profile, f"提取完成但保存失败: {e}"
            ...
            logger.info("文风档案提取完成: %s", status_msg)
            return style_profile, status_msg
        finally:
