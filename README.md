...
- 新增「文风档案提取」功能：StyleProfile 九维文笔风格量化档案（语言质感/叙事节奏/场景构建/人物塑造/情感共鸣/创新签名/主角配角比/视角使用/时间流转），全文一次提取固化到项目，镜像世界观底层三大机制（拆批/增量合并/语义整合）
- 端点配置新增自定义请求扩展：自定义请求体字段（JSON deep merge）+ 自定义 HTTP 头（update），支持 zenmux `provider_routing_strategy` 等场景
- 修复单章续写生成未组装文风档案的 bug，补齐续写预览 4 个档案参数确保预览与实际发送一致
- 统一【写作风格】与【文风档案】概念：主提示词模板【写作风格】改名【写作风格补充说明】并移到【文风档案】正下方，明确量化档案为主、文字描述为辅的主次关系
- 4 个档案（世界观/主角/文风/自定义设定）空值时统一注入占位文本，与审计模板逻辑匹配
- 多个阶段提示词模板（审计/重写/卷续写/写作模式等）补充 `{{style_profile}}` 文风档案占位符
