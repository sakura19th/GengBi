...
- 新增「自定义设定/审计必查项」：用户输入自定义设定（如"主角不得杀人"），AI 结合世界观与上下文结构化为"要求 + 审计向"两字段，项目全局共享，作为审计必查项注入续写生成与审计全链路，未满足 high 严重度项一票否决
- 新增「输出栏高亮」：续写输出框右键菜单支持 4 色高亮（黄/绿/蓝/红）+ 可选备注 + 清除选区/全部，随 swipe 持久化
- 新增「主角形象提取独立按钮」：从上下文提取副产品解耦为独立流程，提供独立的"提取/查看"按钮与独立缓存，与世界观底层提取对称
- 新增「API 端点思考强度」：端点管理新增思考强度配置（不发送/auto/minimal/low/medium/high/max 7 档），预设级优先于端点级，激活预设管理器原有死字段
- 新增「流程端点配置」：7 个流程（单章续写/卷续写/审计/上下文提取/世界观提取/主角提取/自定义设定解析）可独立选择 API 端点，默认沿用端点管理总默认端点
- 新增「自定义设定流式窗口」：新增自定义设定时显示流式输出窗口，与其它提取体验一致
- 编辑器工具栏新增可见「保存」按钮（除原有 Ctrl+S 与 5 秒自动保存）
- 主角形象提取结果落盘到 chapters 表，切换章节/重启进程后可恢复
- 修复编辑保存 API 端点后闪退（QThread 销毁时仍在运行 + aiohttp session 泄漏）
- 修复已有端点无法编辑（clicked(bool) 信号误传 False 给 item 参数）
- 修复章节保存报错 `table chapters has no column named protagonist_profile`（迁移函数已实现但漏调用）
- 修复章节保存失败时正文丢失（回滚改用旧正文恢复而非删文件）
- 修复自定义规则生成时世界观底层序列化失败（datetime 无法序列化被静默吞错）
- 修复自定义设定/世界观提取的 aiohttp session 泄漏与跨线程 GUI 违规（QObject::setParent 卡死）

### v0.2.3

- 优化：世界观底层与上下文提取的每个批次失败后自动重试 1 次（温度归零），避免单次网络波动导致整体提取中止需从头重跑
