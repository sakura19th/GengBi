...
- 新增「重写当前章节」模式：两步流程（结构化分析当前章节正文 + 用户需求 → 检查点暂停审阅 → 复用单章续写链路生成新正文存为 swipe → 接受时替换当前章节正文，不新建章节）；`context_extractor` 与 `prompt_assembler` 新增 `exclude_current` 参数，重写模式下前文提取与聊天历史构建排除当前章节
- 优化重写分析提示词 `phase_rewrite_analysis.txt`：新增「# ⚠️ 重写边界」硬约束块（6 条禁止行为：不推进剧情/不续写下一章/不新增场景/角色/不解决未决伏笔），明确重写边界最高优先原则，用户需求与不推进冲突时标注 conflicts 并建议改用续写模式

### v0.2.5

- 新增 `rigid_ai_text` 审计维度（刻板AI文本禁令，严格给分）：3 处以上 AI 痕迹判 major，5 处以上或整段套路判 critical；单章审计 7 维度、卷级验证 16 维度、大纲审计 13 维度
