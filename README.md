...
https://www.fufukitty.com/index.php/archives/3/
整了一个不同模型在相同提示词下表现的对比，欢迎打分。
...
```bash
# 当前平台打包（生成 dist/GengBi_v{版本号} 可执行文件，如 GengBi_v0.2.16.exe）
...
- 重构「上下文提取预览」面板布局：每个功能合并为一个按钮，点击后弹出操作选择对话框（提取/查看/增量更新/复制到章节等）
- 新增「增量更新」功能：上下文、主角形象、自定义角色均可基于最近的已有提取结果，仅对新增章节做增量提取

### v0.2.17

- 新增「规划写作」流程插件：两步流程（生成细纲→采纳后正文生成），先根据用户指令生成场景级细纲（beats 节拍），用户在审阅对话框中修改或采纳后，细纲作为唯一施工指令传给正文生成阶段

### v0.2.16

- 新增「复制上下文到章节」「复制主角形象/自定义角色到章节」功能，可将提取结果复制到其他章节复用
- 新增「同步注入回溯上下文」「同步注入回溯主角形象/自定义角色」开关（续写面板），开启后续写自动注入回溯窗口内最新一章的提取结果作为独立 system 消息
...
- 修复上下文条目编辑后未持久化导致重启软件后丢失的问题
- 新增条目新增/编辑/删除/清空/禁用 5 类操作的自动保存功能
