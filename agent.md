...
**当前版本：v0.2.17**（定义于 `novelforge/__init__.py` 的 `__version__`，由"关于"对话框引用，README 顶部同步标注）
...
│   ├── context_preview_panel.py # 上下文提取预览面板（ontology/protagonist/custom_character/custom_rule 四类提取按钮互斥 + 流式展示；条目级 enabled 字段为禁用状态真源，_disabled_uids 影子缓存仅 UI 显示用——_on_disable_toggled 同步更新 entry.enabled 与 _disabled_uids，set_entries/_load_entries/_on_extract_finished/_on_entries_loaded 等 4 处入口从 enabled 字段重建 _disabled_uids；新增/编辑/删除/清空/禁用 5 类操作均 emit entries_changed 信号触发 MainWindow 持久化；自定义角色提取按钮弹 QInputDialog 取角色名，查看按钮单角色直接展示/多角色弹选择对话框；功能 1「复制到章节」三类——_copy_to_chapter_btn 按钮 + copy_to_chapter_requested Signal + get_all_entries 返回全部条目（含禁用）用于上下文条目复制；_copy_protagonist_to_chapter_btn 按钮 + copy_protagonist_to_chapter_requested Signal 用于主角形象复制；_copy_custom_character_to_chapter_btn 按钮 + copy_custom_character_to_chapter_requested Signal 用于自定义角色复制；条目元数据 source 描述拼接逻辑：copied_from 非 None 显示「source=复制自第N章（原第X-Y章）」或「source=复制自第N章」，copied_from 为 None 时回退 source_chapter_range「source=第X-Y章」或「source=导入」）
...
- **UI 入口**：`ContextPreviewPanel` 新增「提取自定义角色」「查看自定义角色」按钮（与主角按钮并列，所有提取按钮互斥禁用）。提取按钮弹 `QInputDialog.getText` 取角色名；查看按钮统一弹 master-detail 列表对话框（左侧 QListWidget 角色名按字母排序，右侧 QPlainTextEdit 展示选中角色 8 维度档案，`currentItemChanged` 联动刷新，默认选中第一项），单角色/多角色同一 UI
...
- **测试覆盖**：`tests/test_custom_character_extraction.py`（31 用例，10 测试类：维度过滤/JSON 解析/常量对齐/缓存 key 格式/流式提取单批次+多批次合并+缓存往返+回调+失败/缓存解耦/UI 按钮信号/flow_key 注册/Chapter.custom_characters 字段往返）
...
- **功能 1：复制主角形象/自定义角色到章节**
  ...
  - **UI 入口**：`ContextPreviewPanel` 新增两个按钮 + 信号——`_copy_protagonist_to_chapter_btn`（"复制主角形象到章节"）+ `copy_protagonist_to_chapter_requested` Signal；`_copy_custom_character_to_chapter_btn`（"复制自定义角色到章节"）+ `copy_custom_character_to_chapter_requested` Signal
