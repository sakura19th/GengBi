...
**当前版本：v0.2.16**（定义于 `novelforge/__init__.py` 的 `__version__`，由"关于"对话框引用，README 顶部同步标注）
...
└── resources/       # 资源文件
    ...
    │   ├── jailbreaks/                # 非正文流程破限模板（10 个 jb_*.txt，每文件含 ### LOW/MID/HIGH ### 三档；已全面深化：LOW=认知修正、MID=授权自由、HIGH=沙盒解放，含内部审查对抗锚点）
    ...
    │   └── agent/                   # 续写阶段提示词模板（phase_*.txt；已加入严格遵循用户指令/不引入意外内容约束；所有含 {{world_ontology}} 的模板已加入 {{style_profile}} 文风档案占位符；13 个 phase 模板在 # 自定义设定/审计必查项 段前统一加入 # 自定义角色档案/{{custom_characters}} 段，注入自定义角色形象档案供各阶段参考）
