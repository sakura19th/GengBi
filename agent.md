...
│   ├── agent.py         # 续写流程共享数据模型（Outline/Scene/CritiqueReport/CritiqueIssue + VALID_CRITIQUE_* 常量，被 volume.py 复用）
...
└── resources/       # 资源文件
    ...
    │       ├── phase_verify.txt          # 卷续写复用：验证
    │       ├── phase_revise.txt          # 卷续写复用：修订（含 previous_chapters_text + pacing_speed 占位符）
    ...
    │       ├── phase_outline_final.txt   # 卷续写：终稿大纲生成（基于最后一轮审计+原大纲+前文+深度分析+推进速度）
...
### 8. 卷级多章节续写（Volume-level Multi-chapter Continuation）
