...
│   ├── agent.py         # 续写流程共享数据模型（Outline/Scene/CritiqueReport/CritiqueIssue + VALID_CRITIQUE_* 常量，VALID_CRITIQUE_CATEGORIES 含 10 值对齐 phase_outline_audit/phase_verify 的 10 维度：consistency/pacing/engagement/structure/coherence/foreshadowing/characters/style/protagonist_consistency/worldview_consistency，被 volume.py 复用）
│   └── volume.py        # 卷级多章节续写数据模型（DeepAnalysis/VolumeOutline/VolumeArtifacts/VolumeRunConfig）
├── core/            # 核心逻辑（无 UI 依赖）
│   ├── prompt_assembler.py  # 提示词组装（三阶段：排序→注入→裁剪；worldInfoBefore/After 按 category 分组 Markdown 格式输出，_CATEGORY_LABELS 提供中文标签映射；assemble 加 world_ontology/protagonist_profile/skip_history 三参数，_build_macro_context 将两者序列化为 JSON 字符串填入 MacroContext.extra，skip_history=True 时 _build_history 返回空列表跳过聊天历史构建供卷模式使用，单章与卷级 chapter_writing 共享此链路）
...
│   └── storage.py           # SQLite 异步存储层（aiosqlite；SCHEMA_SQL 建 projects/chapters/continuations/cache 表，chapters/continuations 外键 ON DELETE CASCADE；continuations 表含 parent_id 列 + idx_continuations_parent 索引；_migrate_continuations_columns 补 continuations 表 agent_artifacts/volume_artifacts/parent_id 列、_migrate_projects_columns 补 projects 表 world_ontology/worldbook_id 列；save_project/save_chapter 用 UPSERT（ON CONFLICT DO UPDATE）避免 INSERT OR REPLACE 触发级联删除子表；list_continuations/load_chapters_with_continuations 用显式列名 SELECT（非 SELECT *）保证 row 索引与 INSERT 列序一致；list_chapters 检测 DB 无章节但磁盘有 .txt 文件时自动调 rebuild_chapters_from_disk 从磁盘重建 DB 行；_row_to_continuation row[6]=parent_id 后续字段顺延 + len(row) 防御、_row_to_project 读 row[10]/row[11] 加 len(row) 防御；存储路径、JSON 读写工具）
├── services/        # 业务服务
│   ├── context_extractor.py  # 上下文提取（extract + extract_streaming；多批次 token 拆分 + 【信息汇总】环节 _run_merge_entries，batch_count>1 触发 LLM 合并去重，失败降级 best-effort uid 替换合并；ExtractResult 含 batch_count/merged/protagonist_profile/protagonist_batch_count/protagonist_merged 字段；主角形象提取 _extract_protagonist 复用 batches 划分，逐批增量合并 _merge_protagonist_fields 程序化字段级合并 growth_arc 直接覆盖/其他维度序列化长度启发式，batch_count>1 触发 _run_protagonist_merge 语义整合 2 次重试温度 0.2/0.0 失败降级 accumulated_protagonist，失败不阻塞 8 维度结果；模块级 _filter_protagonist_dimensions/_safe_serialize_dim/_parse_protagonist_response 辅助函数；PROTAGONIST_DIMENSIONS 8 维度常量）
│   ├── ontology_extractor.py # 世界观底层提取器（extract_ontology_streaming 全文拆分分析提取 WorldOntology 7 大维度；镜像 ContextExtractor 三大机制：_split_chapters_by_token_limit 拆批 + 增量更新携带 {{accumulated_ontology}} + batch_count>1 触发 _run_ontology_merge 语义整合，失败降级 accumulated_ontology；2 次重试温度 0.2/0.0；_merge_ontology_fields 程序化字段级合并按序列化长度启发式冲突消解新批次优先；_save_ontology_to_worldbook 拆 7 维度为 ContextEntry category="plot_state" 绑定 project.worldbook_id；提取结果固化到 Project.world_ontology；协程内 await self.storage_service.storage.save_project 直连异步层避免重入死锁，不调用同步 storage_service.save_project）
...
│   ├── volume_orchestrator.py # 卷级多章节续写编排器（QThread+asyncio，独立类；深度分析切分模式追加 _run_deep_analysis_merge LLM 语义整合润色，失败降级返回原 accumulated；__init__ 接收 world_ontology + protagonist_profile 两参数，8 个 phase 方法（deep_analysis/deep_analysis_merge/volume_outline/outline_audit/outline_final/chapter_outline/chapter_verify/chapter_revise）+ chapter_writing assemble 调用均通过 _format_world_ontology/_format_protagonist_profile 格式化为 JSON 注入 {{world_ontology}}/{{protagonist_profile}} 占位符；5 个模板文件 phase_deep_analysis/phase_deep_analysis_merge/phase_outline_final/phase_chapter_outline/phase_revise 已补全对应占位符）
...
│   └── storage_service.py    # 存储服务（项目/章节/续写 CRUD；load_project 把 DB 中的 world_ontology dict 还原为 WorldOntology 实例，失败保留 dict）
├── ui/              # UI 组件（PySide6）
│   ├── main_window.py           # 主窗口（5 栏 QSplitter 布局 + 主题管理 + 调试菜单；OntologyExtractor 实例 + _protagonist_profile_by_chapter 内存 LRU 缓存镜像 _context_entries_by_chapter 模式；4 个 ontology 信号处理方法 _on_extract_ontology_requested 调 context_panel.start_ontology_extraction/_on_ontology_chunk_received 调 update_ontology_progress/_on_ontology_batch_done 调 update_ontology_batch/_on_ontology_done 调 finish_ontology_extraction 或 fail_ontology_extraction 且不再 UI 线程重复 save_project；_on_view_ontology_requested 加载 project.world_ontology 弹 QDialog 按 7 维度分节展示 + _format_ontology_for_display 兼容 dict/WorldOntology 实例；_load_project 解耦——项目元数据加载失败时仍尝试 list_chapters 加载章节表避免章节列表为空；VolumeOrchestrator 构造传 project.world_ontology + 当前章节 protagonist_profile；单章续写 _on_start_continuation 的 prompt_assembler.assemble 调用传 project.world_ontology + _protagonist_profile_by_chapter.get(chapter.id)；单章续写提升为章节模型：_on_accept_continuation 调 promote_continuation_to_chapter 将续写提升为新章节插入当前章节之后并删除续写记录，_refresh_chapter_list 刷新列表后 _on_chapter_selected(new_chapter.id) 自动选中新章节加载到编辑器；_on_delete_continuation 调 storage_service.delete_continuation 删除当前续写记录；续写仅作为待定候选在续写面板可见，不在章节列表中显示；单章续写审计：_on_audit_continuation 加载 phase_single_audit.txt 模板（5 维度精简审计 consistency/style/coherence/protagonist_consistency/worldview_consistency）注入 written_text/user_input/world_ontology/protagonist_profile 4 占位符（_format_world_ontology/_format_protagonist_profile 静态方法 JSON 序列化），AuditWorker 流式输出审计报告到 AuditDialog，_on_audit_accepted 用户编辑后采纳→构造修正 messages（原续写 messages + 原内容 + 审计意见 + 修正要求）新建 ContinuationWorker created_by="audit_rewrite" 流式重写为新 swipe，_on_continuation_finished 保存不删原 swipe；章节切换状态保留：7 个缓冲字段 + 章节守卫 on_origin_chapter + _on_chapter_selected 优先 restore_streaming_state/restore_extraction_state，详见 §12）
│   ├── continuation_panel.py    # 续写控制面板（流式布局按钮区，含审计按钮 + audit_continuation 信号）
│   ├── volume_panel.py          # 卷续写控制面板（卷配置/审计与逐章/暂停点/两层进度/产物；_AUDIT_DIMENSION_LABELS 含 10 维度含 protagonist_consistency主角一致性/worldview_consistency世界观一致性，复选框网格自动迭代 DEFAULT_AUDIT_DIMENSIONS）
...
└── resources/       # 资源文件
    ...
    │       ├── phase_verify.txt          # 卷续写复用：验证（10 维度对齐 phase_outline_audit；protagonist_consistency/worldview_consistency 为必审项；7 处反复强调必审；summary 强制含【主角一致性审计】/【世界观一致性审计】固定标记段落；worldview_consistency 8 条检查项对齐 WorldOntology 7 大维度）
    │       ├── phase_revise.txt          # 卷续写复用：修订（含 previous_chapters_text + pacing_speed + world_ontology + protagonist_profile 占位符）
    │       ├── phase_deep_analysis.txt   # 卷续写：前文深度分析（DeepAnalysis，含 world_ontology + protagonist_profile 占位符）
    │       ├── phase_deep_analysis_merge.txt  # 卷续写：深度分析【信息汇总】环节（切分模式下 LLM 语义整合润色，9 占位符含 world_ontology + protagonist_profile）
...
- 数据模型定义于 `novelforge/models/volume.py`（镜像 agent.py 的 pydantic v2 风格）：
  ...
  - `OutlineAuditReport`/`AuditDimension`：大纲审计，默认 8 维度（consistency/pacing/engagement/structure/coherence/foreshadowing/characters/style），新增 style（文风：文风/语言特色/叙事视角是否与前文统一）
...
- 6 阶段独立提示词模板（resources/defaults/agent/phase_*.txt）：
  ...
  - phase_deep_analysis_merge.txt：深度分析【信息汇总】环节提示词（切分模式下 LLM 语义整合润色），7 个占位符（6 个 profile 宏 + `{{deep_analysis}}` 注入程序化合并后的 DeepAnalysis JSON）；四大整合任务：消除跨块重复、冲突消解、补全与连贯、非模板化；输出严格 DeepAnalysis JSON 对象；编排器 `_run_deep_analysis_merge` 加载此模板，2 次重试（温度 0.2/0.0），失败降级返回原 accumulated 不阻塞卷续写流程
...
- 编排器 `novelforge/services/volume_orchestrator.py` 的 `VolumeOrchestrator` 采用 QThread+asyncio 模式（独立类，不继承，通过组合持有 PromptAssembler，复用 post_process_content）：
  ...
  - **after_chapter 暂停点**（可选，`checkpoints["after_chapter"]` 开启时）：verify/revise 循环结束后触发 `_wait_for_resume_with_chapter` emit `checkpoint_reached("after_chapter", {chapter_index, content})`，UI 弹 `ChapterConfirmDialog`。用户 approve → 继续下一章；reject → 用户反馈构造为 `user_guidance`（`_run_chapter_revise` 新增 `user_guidance` 参数，非 None 时短路 LLM 调用直接返回）→ `_run_chapter_writing` 重写 → `_run_chapter_verify` 重新验证 → 自动修订循环 → 再次触发 after_chapter；cancel → 抛 CancelledError 终止。无最大不通过次数限制
...
- **精简审计模板** `resources/defaults/agent/phase_single_audit.txt`：5 维度（consistency/style/coherence/protagonist_consistency/worldview_consistency），后两者为必审项（一票否决/严格给分），4 占位符（`{{written_text}}`/`{{user_input}}`/`{{world_ontology}}`/`{{protagonist_profile}}`），不含 snapshot/outline/previous_chapters_text（卷续写专用）；输出严格 JSON（summary/issues/passed），summary 必含 `【主角一致性审计】`与`【世界观一致性审计】`标记段落
...
- **修正**（`MainWindow._on_audit_accepted`）：采纳后构造修正 messages = 原 `_continuation_prompt_messages` + 原续写内容（user 消息）+ 审计意见 + 修正要求（user 消息）→ 清空输出框 start_streaming → 新建 ContinuationWorker `created_by="audit_rewrite"` 流式重写 → `_on_continuation_finished` 保存为**新 swipe**（不删原 swipe，可并排对比）；审计报告以纯文本（非结构化 JSON）拼入修正提示词，即使原报告 JSON 不完整或用户改坏仍可发送
- **数据来源**：written_text=current_swipe.content、user_input=continuation_panel.get_user_input()（空则占位）、world_ontology=project.world_ontology（None 占位）、protagonist_profile=_protagonist_profile_by_chapter.get(chapter.id)（None 占位）；原续写 messages 复用 _continuation_prompt_messages 快照（上次续写记录）
