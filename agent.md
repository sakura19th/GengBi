...
├── core/            # 核心逻辑（无 UI 依赖）
│   ├── prompt_assembler.py  # 提示词组装（三阶段：排序→注入→裁剪；worldInfoBefore/After 按 category 分组 Markdown 格式输出，_CATEGORY_LABELS 提供中文标签映射；assemble 加 world_ontology/protagonist_profile 两参数，_build_macro_context 将两者序列化为 JSON 字符串填入 MacroContext.extra，单章与卷级 chapter_writing 共享此链路）
