...
│   ├── context_preview_panel.py # 上下文提取预览面板（ontology/protagonist/custom_rule 三类提取按钮互斥 + 流式展示；条目级 enabled 字段为禁用状态真源，_disabled_uids 影子缓存仅 UI 显示用——_on_disable_toggled 同步更新 entry.enabled 与 _disabled_uids，set_entries/_load_entries/_on_extract_finished/_on_entries_loaded 等 4 处入口从 enabled 字段重建 _disabled_uids；新增/编辑/删除/清空/禁用 5 类操作均 emit entries_changed 信号触发 MainWindow 持久化）
