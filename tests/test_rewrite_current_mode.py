...
class TestFlowEndpointRewriteAnalysis:
    ...
    def test_flow_definitions_count_is_13(self) -> None:
        """FLOW_DEFINITIONS 含 13 个流程（原 7 个 + rewrite_analysis + writing_element_analysis + writing_element_refinement + style_extraction + custom_character_extraction + ontology_extraction + planned_writing_outline）。"""
        from novelforge.ui.flow_endpoint_dialog import FLOW_DEFINITIONS

        assert len(FLOW_DEFINITIONS) == 13, (
            f"FLOW_DEFINITIONS 应有 13 项，实际: {len(FLOW_DEFINITIONS)}"
        )
