...
镜像 ``test_incremental_protagonist_extraction.py`` 的结构，覆盖：
1. find_custom_character_base 基准查找：命中 / 未命中（角色名不匹配）/ 无档案 / 排除当前章
2. extract_custom_character_streaming 增量：delta 章节选择
3. initial_accumulated 基准档案注入首批提示词（{{accumulated_protagonist}}）
4. 结果落盘缓存元数据 incremental_base_index + character_name
"""
...
def _make_project_with_chapters(chapter_count: int) -> tuple[Any, list[Any]]:
    ...
    project = Project(id="inc_custom_proj", name="测试小说", novel_profile=profile)
    chapters = [
        Chapter(
            id=f"ch_{i}",
            project_id="inc_custom_proj",
            index=i,
            title=f"第{i + 1}章",
...
# ===== 1. find_custom_character_base 基准查找 =====


class TestFindCustomCharacterBase:
    """find_custom_character_base 基准查找测试。"""

    def test_find_custom_character_base_hit(self) -> None:
        """最近的含指定角色档案的前章作为基准。"""
        extractor = _make_extractor()
        _, chapters = _make_project_with_chapters(4)
        chapters[1].custom_characters = {
            "林婉": ProtagonistProfile(basic_anchors={"name": "林婉"})
        }
        current_chapter = chapters[3]

        result = extractor.find_custom_character_base(
            chapters=chapters,
        ...
        assert base_profile.basic_anchors == {"name": "林婉"}

    def test_find_custom_character_base_character_mismatch(self) -> None:
        """前章有档案但角色名不匹配时返回 None。"""
        extractor = _make_extractor()
        _, chapters = _make_project_with_chapters(3)
        chapters[1].custom_characters = {
            "苏沐": ProtagonistProfile(basic_anchors={"name": "苏沐"})
        }
    ...
    def test_find_custom_character_base_miss(self) -> None:
        """前章均无该角色档案时返回 None。"""
        extractor = _make_extractor()
        _, chapters = _make_project_with_chapters(3)
        current_chapter = chapters[2]
    ...
    def test_find_custom_character_base_excludes_current(self) -> None:
        """当前章自身的档案不作为基准。"""
        extractor = _make_extractor()
        _, chapters = _make_project_with_chapters(3)
        # 仅当前章有该角色档案
        chapters[2].custom_characters = {
            "林婉": ProtagonistProfile(basic_anchors={"name": "林婉"})
        }
        current_chapter = chapters[2]
    ...
    def test_find_custom_character_base_skips_none_custom_characters(self) -> None:
        """中间章节无 custom_characters 时跳过继续往前找。"""
        extractor = _make_extractor()
        _, chapters = _make_project_with_chapters(4)
        chapters[2].custom_characters = None  # 无档案
        chapters[1].custom_characters = {
            "林婉": ProtagonistProfile(basic_anchors={"name": "早期林婉"})
        }
        current_chapter = chapters[3]
...
# ===== 2. extract_custom_character_streaming 增量 =====


class TestIncrementalCustomCharacterExtraction:
    """自定义角色增量提取测试。"""

    def test_extract_custom_character_incremental_delta_and_seed(self) -> None:
        """delta 章节 + 基准档案注入首批提示词 + 缓存元数据。"""
        extractor = _make_extractor()
        client = _CustomCharacterLLMClient()
        client.add_stream_response(_profile_json("增量后林婉"))
        ...
        base_profile = ProtagonistProfile(basic_anchors={"name": "基准林婉"})
        ...
        assert "基准林婉" in client.prompts[0]
        # prompt 含角色名
        assert "林婉" in client.prompts[0]

        # 缓存元数据含 incremental_base_index + character_name
        ...
        assert call_args.args[0].startswith("custom_character:")

    def test_extract_custom_character_incremental_current_not_after_base(self) -> None:
        ...
        assert profile is not None
        assert profile.basic_anchors == {"name": "基准林婉"}
        assert "未执行提取" in status
        assert client.stream_call_count == 0

    def test_extract_custom_character_incremental_multi_batch(self) -> None:
        """小 token_limit 触发多批次拆分，首个批次 prompt 携带基准档案。"""
        extractor = _make_extractor()
        client = _CustomCharacterLLMClient()
        # 3 个提取批次（3 章 delta 各拆 1 批）+ 1 次合并
        for i in range(3):
            client.add_stream_response(_profile_json(f"批次{i}林婉"))
        client.add_stream_response(_profile_json("合并后林婉"))
        extractor._get_llm_client = lambda flow_key="": (client, "gpt-4o-mini")  # type: ignore[assignment]
        ...
        assert profile is not None
        assert "完成" in status
        # 首个提取批次 prompt 携带基准档案（initial_accumulated 种子）
        assert "基准林婉" in client.prompts[0]
        # 后续批次携带前批次累积结果
        assert "批次0林婉" in client.prompts[1]
        assert "批次1林婉" in client.prompts[2]
